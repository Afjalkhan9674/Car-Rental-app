  ...
  return (
    ...
    xl:px-32 py-4 text-gray-600 border-b border-borderColor relative transition-all ${location.pathname === "/" && "bg-light"}`}>
      ...
      <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor 
       ...
       ${location.pathname === "/" ? "bg-light" : "bg-white"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"} ${opne ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
        {menuLinks.map((link, index) => (
          <Link  onClick={() => setOpne(!opne)} key={index} to={link.path}>
            {link.name}
          </Link>
        ))}
        ...
        <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
          ...
          <button onClick={() => setShowLogin(true)} className='cursor-pointer px-8 py-2 bg-primary hover:bg-black transition-all text-white rounded-lg'>Login</button>
        </div>
      </div>

      <button className='sm:hidden cursor-pointer' aria-label='Menu' onClick={() => setOpne(!opne)}>
        <img src={opne ? assets.close_icon : assets.menu_icon} alt="Menu" />
      </button>
