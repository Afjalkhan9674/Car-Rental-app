...
const currency = import.meta.env.VITE_CURRENCY
  ...
  const dashboardCards = [
    ...
    { title: "Connfirmed", value: data.completedBookings, icon: assets.listIconColored },
  ]
          ...
          {data.recentBookings.map((booking, index)=>(
           <div key={index} className='mt-4 flex items-center justify-between'>
             <div className='flex items-center gap-2'>
              ...
              <div>
                <p>{booking.car.brand} {booking.car.modle}</p>
             ...
             <div className='flex items-center gap-2 font-medium'>
               ...
               {/* {`px-3 py-1 text-xs rounded-full ${booking.status === 'confirmed' ? 'bg-green-400/15 text-green-600' : 'bg-red-400/15 text-red-600'}`} */}
             </div>
           </div>
          ))}
        </div>
